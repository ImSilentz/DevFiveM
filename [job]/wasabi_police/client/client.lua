        ...
        if escorting?.active then
            ...
            if DoesEntityExist(targetPed) and IsPedOnFoot(targetPed) and not IsPedDeadOrDying(targetPed, true) then
                if not alrEscorting then
                    ...
                    TaskPlayAnim(cache.ped, 'amb@code_human_wander_drinking_fat@beer@male@base', 'static', 8.0, 1.0, -1, 49, 0, false, false, false)
                    alrEscorting = true
    ...
    for k,v in pairs(Config.Locations) do
                ...
                CreateThread(function()
                    ...
                    function point:nearby()
                        ...
                        elseif wsb.framework == 'esx' then
                            jobName, jobGrade = wsb.hasGroup('off'..self.job)
        ...
        if v.bossMenu.enabled then
            if v.bossMenu?.target?.enabled then
                ...
                if pJob then
                    wsb.target.boxZone(k..'_pdboss', v.bossMenu.target.coords, v.bossMenu.target.width, v.bossMenu.target.length, {
